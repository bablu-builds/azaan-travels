  ...
  const allDues = useMemo<DueItem[]>(() => {
    ...
    aepsEntries.forEach(e => {
      ...
      items.push({
        ...
        addedBy: e.addedBy || '',
        settle: async (mode, settledBy) =>
          settlePendingEntry('aeps', e.id!, mode, settledBy, { amount: e.amount, customerName: e.customerName ?? 'Customer', category: 'AEPS Withdrawal' }),
      });
    });
    ...
    rechargeEntries.forEach(e => {
      ...
      items.push({
        ...
        addedBy: e.addedBy || '',
        settle: async (mode, settledBy) =>
          settlePendingEntry('recharge', e.id!, mode, settledBy, { amount: e.rechargeAmount, customerName: e.customerName ?? 'Customer', category: 'Electric Recharge' }),
      });
    });
    ...
    flightEntries.forEach(e => {
      ...
      items.push({
        ...
        addedBy: e.addedBy || '',
        settle: async (mode, settledBy) =>
          settlePendingEntry('flight', e.id!, mode, settledBy, { amount: e.amountCharged, customerName: e.customerName ?? 'Customer', category: 'Flight Booking' }),
      });
    });
