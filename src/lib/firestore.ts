...
import { PaymentMode, PaymentStatus, SettlementMode, deriveStatus, resolveStatus } from '@/lib/payments';
...
type GeneratedTransactionFields =
  | 'id'
  | 'entryNumber'
  | 'type'
  | 'details'
...
const asTimestamp = (value: unknown): Timestamp =>
  value instanceof Timestamp ? value : Timestamp.fromMillis(0);

const asPaymentMode = (value: unknown): PaymentMode | undefined =>
  value === 'Cash' || value === 'Online' || value === 'Due' || value === 'None'
    ? value
    : undefined;

const asSettlementMode = (value: unknown): SettlementMode | undefined =>
  value === 'Cash' || value === 'Online' ? value : undefined;

const asPaymentStatus = (value: unknown): PaymentStatus | undefined =>
  value === 'paid' || value === 'pending' || value === 'free' ? value : undefined;

function viewTransaction(type: 'aeps', id: string, data: Record<string, unknown>): AepsWithdrawal;
function viewTransaction(type: 'recharge', id: string, data: Record<string, unknown>): ElectricRecharge;
function viewTransaction(type: 'transfer', id: string, data: Record<string, unknown>): MoneyTransfer;
function viewTransaction(type: 'flight', id: string, data: Record<string, unknown>): FlightBooking;
function viewTransaction(type: 'quickWork', id: string, data: Record<string, unknown>): QuickActionEntry;
function viewTransaction<T extends TransactionType>(
  type: T,
  id: string,
...
function viewTransaction(
  ...
  const base: TransactionRecord = {
    id,
    entryNumber: asString(data.entryNumber),
    type,
    customerName: asOptionalString(data.customerName),
    amount: asNumber(data.amount),
    profitMargin: asNumber(data.profitMargin),
    paymentStatus: resolveStatus(asPaymentStatus(data.paymentStatus)),
    paymentMode: asPaymentMode(data.paymentMode),
    settledVia: asSettlementMode(data.settledVia),
    settledBy: asOptionalString(data.settledBy),
    settledAt: data.settledAt instanceof Timestamp ? data.settledAt : undefined,
    createdAt: asTimestamp(data.createdAt),
    addedBy: asString(data.addedBy),
  ...
  if (type === 'aeps') {
    return {
      ...base,
      bankName: asString(details.bankName),
      mobile: asOptionalString(details.mobile),
    };
  }
  if (type === 'recharge') {
    return {
      ...base,
      rechargeAmount: base.amount,
      consumerNumber: asString(details.consumerNumber),
      mobile: asOptionalString(details.mobile),
    };
  }
  if (type === 'transfer') {
    return {
      ...base,
      name: base.customerName ?? 'Customer',
      mobileOrAccount: asString(details.mobileOrAccount),
    };
  }
  if (type === 'flight') {
    return {
      ...base,
      actualFare: asNumber(details.actualFare),
      amountCharged: base.amount,
      flightFrom: asString(details.flightFrom),
      flightTo: asString(details.flightTo),
      boardingDate: asString(details.boardingDate),
    };
  }
  return {
    ...base,
    category: asString(details.category, 'Other') as QuickActionCategory,
  };
}
...
export const subscribeTransactions = <T extends TransactionType>(
  ...
  return onSnapshot(q, (snap) => {
    callback(snap.docs.map(d => viewTransaction<T>(type, d.id, d.data() as Record<string, unknown>)));
  }, (err) => {
...
export const createAepsWithdrawal = async (
  data: Omit<AepsWithdrawal, GeneratedTransactionFields>,
...
export const createElectricRecharge = async (
  data: Omit<ElectricRecharge, GeneratedTransactionFields | 'amount'>,
...
export const createMoneyTransfer = async (
  data: Omit<MoneyTransfer, GeneratedTransactionFields>,
...
export const createQuickAction = async (
  data: Omit<QuickActionEntry, GeneratedTransactionFields | 'profitMargin'>,
...
export const createFlightBooking = async (
  data: Omit<FlightBooking, GeneratedTransactionFields | 'amount'>,
